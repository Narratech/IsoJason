...
public class get_direction extends DefaultInternalAction {
    ...
    public Object execute(TransitionSystem ts, Unifier un, Term[] terms) throws Exception {
        try {
            ...
            WorldState model = WorldState.get();
...
class GridState implements Estado, Heuristica {
    ...
    public GridState(Location l, Location from, Location to, WorldState model, String op) {
        this.pos = l;
        this.from = from;
        this.to = to;
        this.model = model;
    ...
    private void suc(List<Estado> s, Location newl, String op) {
        if (model.isFree(newl.x,newl.y) || (from.distance(newl) > 3 && model.isFreeOfObstacle(newl.x,newl.y))) {
