...
////// Specular:
                ...
                float4 node_6771 = _Time + _TimeEditor;
                float2 node_4572 = (i.uv0+node_6771.g*float2(0.0005,0)); // Cloud speed
...
////// Emissive:
                ...
                float3 _Nightemissivecities_var = lerp( float3(0,0,0), (_Citycolor.rgb*(5.0*(_Emissive_var.rgb*node_7821))), _Nightemissivecities ); // Cities emissive
                float3 emissive = (_Nightemissivecities_var+((lerp( float3(0,0,0), (pow(1.0-max(0,dot(normalDirection, viewDirection)),_SubAtmospherepower)*node_3504), _SubAtmosphere )*_Subatmospherecolor.rgb)*_Subatmosphereintensity));
/// Final Color:
...
////// Specular:
                float NdotL = max(0, dot( normalDirection, lightDirection ));
                float4 node_9855 = _Time + _TimeEditor;
                float2 node_4572 = (i.uv0+node_9855.g*float2(0.0005,0)); // Cloud speed
                float4 _Cloudstexture_var = tex2D(_Cloudstexture,TRANSFORM_TEX(node_4572, _Cloudstexture));
                float node_2618 = (1.0 - _Cloudstexture_var.a);
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(i.uv0, _Specular));
                float3 specularColor = (node_2618*(_Specularcolor.rgb*_Specular_var.rgb));
